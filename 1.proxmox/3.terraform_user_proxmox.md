# 🔐 Creating a Terraform User and Token in Proxmox VE 9
...
> **⚠️ Important:** `VM.Config.HWType` is **required** for serial console and VGA configuration. Without it, VM creation will fail.

---

### 2️⃣ Create the Terraform User
...
- ✅ All future templates you create
...
> **⚠️ ESSENTIAL:** The `--privsep 0` flag is **mandatory**! Without it, the token won't inherit user permissions and will fail with 403 errors.
...
│ key          │ value                                           │
╞══════════════╪═════════════════════════════════════════════════╡
│ full-tokenid │ terraform1@pve!tf                               │
├──────────────┼─────────────────────────────────────────────────┤
│ info         │ {"comment":"Terraform API token","privsep":"0"} │
├──────────────┼─────────────────────────────────────────────────┤
│ value        │ xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx            │
└──────────────┴─────────────────────────────────────────────────┘
```

**⚠️ SAVE THE TOKEN VALUE IMMEDIATELY!**
...
╞═════════╪═════════════════════╪════════╪═════════╡
│ tf      │ Terraform API token │      0 │ 0       │  ← Must be 0!
└─────────┴─────────────────────┴────────┴─────────┘
```
...
- ✅ Proxmox VE 9 (Trixie)
- ✅ 2-node cluster with non-shared storage
- ✅ Multiple template deployment
- ✅ K3s cluster deployment (5 VMs)
- ✅ Consul state backend integration
