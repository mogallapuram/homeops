# ☁️ Creating a Cloud-Init Image (Ubuntu 25.10 - Plucky Puffin)
...
```bash
mv mv noble-server-cloudimg-amd64.img ubutu-24.04.qcow2
```

### 3️⃣ Upgrade the Image
```bash
virt-customize -a ubutu-24.04.qcow2 --run-command 'apt-get update && apt-get -y dist-upgrade'
```
...
```bash
virt-customize -a ubutu-24.04.qcow2 --install qemu-guest-agent
virt-customize -a ubutu-24.04.qcow2 --install ssh
```

### 5️⃣ Resize the Image
Resize the VM disk to the required size (example: **30G**):
```bash
qemu-img resize ubutu-24.04.qcow2 30G
```
...
### 7️⃣ Import the Disk
```bash
qm disk import 8000 ubutu-24.04.qcow2 local-lvm
```
