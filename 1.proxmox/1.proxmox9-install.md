...
### Disable ALL enterprise repos (both .list and .sources)
```bash
sed -i -E 's|^(deb .*enterprise\.proxmox\.com.*)$|# \1|' /etc/apt/sources.list /etc/apt/sources.list.d/*.list 2>/dev/null || true

for f in /etc/apt/sources.list.d/*.sources; do
  if grep -q 'enterprise\.proxmox\.com' "$f"; then
    echo "Disabling $f"
    sed -i -E 's/^Enabled:\s*yes/Enabled: no/' "$f"
  fi
done


### Add no-subscription repo (PVE 9 = trixie)
...
## 8️⃣ Optional – Apply a Black Theme (Dark Mode for Proxmox)  ---not working as of nov 7th 2025
